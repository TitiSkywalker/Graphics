...
Vector3f Vector3f::clamp(const Vector3f& v)
{
	float x = (v[0] < 0.0f) ? 0.0f : v[0];
	float y = (v[1] < 0.0f) ? 0.0f : v[1];
	float z = (v[2] < 0.0f) ? 0.0f : v[2];
...
float Vector3f::clampedDot(const Vector3f& v0, const Vector3f& v1)
{
...
Vector3f Vector3f::pointwiseDot(const Vector3f v0, const Vector3f& v1)
{
